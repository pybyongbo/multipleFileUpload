...
async function addWatermark(imagePath, watermarkText) {
  try {
    ...
    let watermarkHeight = Math.min(watermarkMetadata.height, metadata.height - 20);
    
    // 如果水印仍然太大，重新调整
    if (watermarkWidth > metadata.width - 20 || watermarkHeight > metadata.height - 20) {
...
exports.getFileListByUserId = async (ctx) => {
  ...
  try {
    const files = await fileModel.getFileListByUserId(userId,page);
    const total = await fileModel.getFileActiveCountByUserId(userId); // 总记录数
...
exports.batchDeleteFile = async (ctx) => { 
  ...
  console.log('ids',fileList.map(item => item.id));
...
exports.getOtherFileListTop5 = async (ctx) => { 
  ...
  try {
      const userId = ctx.state.user.id;
      const fileOtherListPicTop5 = await fileModel.getOtherFileListPicTop5(userId);
...
exports.uploadFileBase64 = async (ctx) => {
  try {
    ...
    const uniqueFileName = `${path.basename(fileName, ext) || 'file'}-${Date.now()}${ext}`;
    ...
    if (fileInfo.mimeType.startsWith('image/')) {
      try {
        ...
        fileInfo.full_path = `${ctx.request.protocol}://${ctx.request.host}/uploads/${path.basename(watermarkedImagePath)}`;
...
exports.uploadFileBinary = async (ctx) => {
  try {
    ...
    if (ctx.request.type === 'application/octet-stream' || 
      ...
      // 从请求头中获取文件信息
      const fileName = decodeURIComponent(ctx.request.headers['x-file-name'] || 'file.bin');
      const fileSize = parseInt(ctx.request.headers['x-file-size']) || binaryData.length;
      const mimeType = ctx.request.headers['x-file-type'] || 'application/octet-stream';
