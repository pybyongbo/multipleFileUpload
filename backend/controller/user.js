...
// console.log(1111, md5('pyb901214'));
...
exports.insertUserData = async (ctx) => {
  try {
    ...
    const is_active =
      ctx.request.body.is_active !== undefined ? ctx.request.body.is_active : 0;
    ...
    if (err.code === 'ER_DUP_ENTRY') {
      ...
      ctx.body = resObj(
        500,
        {},
        `服务器内部错误,${process.env.NODE_ENV} === 'development' ? ${err.message} : undefined`,
      );
      // ctx.body = {
      //   code: 500,
      //   message: '服务器内部错误',
...
exports.updateUserAllInfo = async (ctx) => {
  try {
    const { id, username, nickname, phonenumber, email, gender, is_active } =
      ctx.request.body;
    await userModel.updateUserAllInfo({
      id,
      username,
      nickname,
      phonenumber,
